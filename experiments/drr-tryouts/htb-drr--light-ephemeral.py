...
def start_nodes(delay_router, loss_router, quicserver, quicclient, mqs, trafficserver, trafficclient, apps):
 ...
 info('\n (QUIC) CMD: tc class add dev hu-eth0 parent 2: classid 2:1 drr quantum 55\n')
 quicclient.cmd('tc class add dev hu-eth0 parent 2: classid 2:1 drr quantum 55') # quic quantum
 quicclient.cmd('tc filter add dev hu-eth0 protocol ip parent 2:0 prio 1 u32 match ip dport 6121 0xffff classid 2:1') 
 # quic filter with destination port 6121, class 2:1

# CLASSES for UDP traffic (class 2:2 to 2:apps+1)
 for i in range(apps):
	 info('\n (UDP) CMD: tc class add dev hu-eth0 parent 2: classid 2:' + str(i+2) + ' drr quantum 1400\n')
 	 quicclient.cmd('tc class add dev hu-eth0 parent 2: classid 2:' + str(i+2) + ' drr quantum 1400') #TODO: quantum for crosstraffic?
	 quicclient.cmd('tc filter add dev hu-eth0 ip parent 2:0 prio 1 u32 match ip dport 600'+ str(i+1) +'0xffff classid 2:' + str(i+2))
 ...
 quicclient.cmd('sleep 5; sudo ./start_client_mininet.sh 6000 100 100 L > stdoutput_client.txt &')
 info( '\n*** QUIC Client started.\n' )

 # UDP crosstraffic: "apps" number of UDP applications
 for i in range(apps): #starttimes: 60,90,120,150 | runtime = 150 sec | endtimes: 210, 240, 270, 300
	time.sleep(30)
