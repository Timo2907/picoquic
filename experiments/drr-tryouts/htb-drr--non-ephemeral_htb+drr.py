...
# FILTERS for QUIC + UDP traffic
 quicclient.cmd('tc filter add dev hu-eth0 parent 1: prio 1 u32 match ip dport 6121 0xffff classid 2:1') 
 # quic filter with destination port 6121, class 2:1
 for j in range(apps):
	 quicclient.cmd('tc filter add dev hu-eth0 parent 1: prio 1 u32 match ip dport 600'+ str(j+1) +' 0xffff classid 2:' + str(j+2))
 ...
 quicclient.cmd('tc class show dev hu-eth0 >> tc.log')
