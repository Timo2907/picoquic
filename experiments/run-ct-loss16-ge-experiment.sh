#!/bin/bash
########################### Crosstraffic + GE Loss 16 Experiments ################################
for i in {1..10}
do
    echo "Welcome $i out of 10 times"

    sleep 5
    experiment='ct-ge-loss16--non-ephemeral'
    mn --clean
    ...
    sleep 5
    experiment='ct-ge-loss16--light-ephemeral'
    mn --clean
    echo "##### CLEANED UP MININET #####"
    start=`date +%s`
    echo "Start time:" $(date) "of experiment" $experiment
    ...
    sleep 5
    experiment='ct-ge-loss16--full-ephemeral'
    mn --clean
    echo "##### CLEANED UP MININET #####"
    start=`date +%s`
    echo "Start time:" $(date) "of experiment" $experiment
