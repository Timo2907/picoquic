...
def start_nodes(delay_router, loss_router, server, client, mqs, trafficserver, trafficclient):
 ...
 trafficserver.cmd('sudo iperf3 -s -p 6666 > trafficserver.log &')
 info( '\n*** Start TRAFFIC Client...\n' )
 trafficclient.cmd('iperf3 -c 10.20.0.1 -p 6666 -t 25 -u -b 10MB >> trafficclient.log&')
 #trafficclient.cmd('sudo for run in {1..10}; do iperf3 -c 10.20.0.1 -p 6666 -t 25 -u -b 10MB >> trafficclient.log; sleep 5; done &')
 ...
 client.cmd('sleep 5; sudo ./start_client_mininet.sh 3000 100 100 N > stdoutput_client.txt &')
 info( '\n*** QUIC Client started.\n' )
