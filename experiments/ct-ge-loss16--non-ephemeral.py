...
def start_nodes(delay_router, loss_router, server, client, mqs, trafficserver, trafficclient):
 ...
 delay_router.cmd('tc qdisc add dev rh-eth1 root netem loss gemodel 1.892% 10% 90% 2% limit ' + str(mqs))
 #move-to-burstmode (p) of 1.892%, move-to-gapmode (r) of 10%, drop-in-burstmode (1-h) of 90% and drop-in-gapmode (1-k) of 2%
 ...
 trafficserver.cmd('sudo iperf -s -p 6666 > trafficserver.log &')
 info( '\n*** Start TRAFFIC Client...\n' )
 trafficclient.cmd('sudo iperf -c 10.20.0.1 -p 6666 -t 610 >> trafficclient.log&')
 ...
 client.cmd('sleep 5; sudo ./start_client_mininet.sh 6000 100 100 N > stdoutput_client.txt &')
 info( '\n*** Client started.\n' )
