#!/bin/bash
########################### Crosstraffic with DRR Experiments ################################
for i in {1..1}
do
    echo "Welcome $i out of 1 times"

    sleep 5
    experiment='htb-drr--non-ephemeral-dynamic'
    mn --clean
    ...
    sleep 5
    experiment='htb-drr--light-ephemeral-dynamic'
    mn --clean
    echo "##### CLEANED UP MININET #####"
    start=`date +%s`
    echo "Start time:" $(date) "of experiment" $experiment
    ...
    sleep 5
    experiment='htb-drr--full-ephemeral-dynamic'
    mn --clean
    echo "##### CLEANED UP MININET #####"
    start=`date +%s`
    echo "Start time:" $(date) "of experiment" $experiment
