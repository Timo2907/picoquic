...
# default filter
 ...
 quicclient.cmd('sleep 5; sudo ./start_client_mininet.sh 6000 2000 100 L > stdoutput_client.txt &')
 info( '\n*** QUIC Client started.\n' )
 ...
 for i in range(apps):
	...
	if i<apps-1: # starttime: 120, on-off pattern, 4x 10 sec on, 5 sec off
		info( '\n*** Start UDP Server', i+1,' on port 600'+ str(i+1) +' ...\n' )
 		...
 		quicclient.cmd('sudo iperf -c 10.20.0.1 -p 600'+ str(i+1) +' -t 250 -u -b 250k > trafficclient'+ str(i+1) +'.log&')
	else:
		for j in range(4):
			info( '\n*** Start dynamic UDP Server', i+1,' on port 600'+ str(i+1) +' ...\n' )
 			trafficserver.cmd('sudo iperf -s -p 600'+ str(i+1) +' -u >> trafficserver'+ str(i+1) +'.log &')
 			info( '\n*** Start dynamic UDP App', i+1,' on QUIC Client for Server on port 600'+ str(i+1) +' ...\n')
 			quicclient.cmd('sudo iperf -c 10.20.0.1 -p 600'+ str(i+1) +' -t 10 -u -b 250k >> trafficclient'+ str(i+1) +'.log&')
			time.sleep(15)
