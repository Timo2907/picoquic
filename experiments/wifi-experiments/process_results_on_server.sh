...
grep -E "(Stream.*length 100)" $path/$dirname/log_server_*.txt | awk '{print $2, $5, $8, $11, $19}' > $path/$dirname/serverStreamOutput
# Reception Time in sec, Stream ID, payload in bytes, msg number
grep -E "(Server CB.*100 bytes)" $path/$dirname/log_server_*.txt | awk '{print $2, $7, $9, $15}' > $path/$dirname/serverAppOutput